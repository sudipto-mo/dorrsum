...
export async function GET(request: NextRequest) {
  ...
  if (!clientId || !clientSecret || !redirectUri || !authSecret) {
    return withClear(redirectToWorkbench(request, { linkedin_auth: "missing_config" }));
  }
  ...
  if (q.error) {
    ...
    return withClear(
      redirectToWorkbench(request, { linkedin_auth: "error", reason: msg.slice(0, 200) })
    );
  }
  ...
  try {
    ...
    return withClear(
      redirectToWorkbench(request, { linkedin_auth: "error", reason: "Token request failed" })
    );
  }
  ...
  if (!tokenRes.ok || !tokenJson.access_token) {
    ...
    return withClear(redirectToWorkbench(request, { linkedin_auth: "error", reason: liErr }));
  }
  ...
  if (!profile.sub) {
    ...
    return withClear(redirectToWorkbench(request, { linkedin_auth: "error", reason: detail }));
  }
  ...
  const res = redirectToWorkbench(request, { linkedin_auth: "success" });
